...
def fetch_damai(): url = "https://search.damai.cn/search.html?keyword=&destCity=%E4%B8%8A%E6%B5%B7&projectType=1" out = [] try: r = requests.get(url, headers={"User-Agent": USER_AGENT}, timeout=30) r.raise_for_status() soup = BeautifulSoup(r.text, "html.parser") items = soup.select("a[href*='detail'], a[href*='item']") seen = set() for a in items: href = a.get("href") or "" if not href: continue if href.startswith("//"): href = "https:" + href if href and not href.startswith("http"): href = "https://search.damai.cn/" + href.lstrip("/") title = (a.get_text(" ", strip=True) or "").strip() if not title or href in seen: continue seen.add(href) card = a.find_parent(["div", "li"]) or a meta = card.get_text(" ", strip=True) date_text = "" m = re.search(r"(20\d{2}[-./年]\s?\d{1,2}[-./月]\s?\d{1,2})", meta) if m: date_text = m.group(1) clean_date = date_text.replace("年", "-").replace("月", "-").replace("日", "") dt = parse_date(clean_date, "Asia/Shanghai") date_local = dt.strftime("%Y-%m-%d %H:%M") if dt else "" status = "on_sale" if ("立即购买" in meta or "购票" in meta) else "announced" out.append({ "source": "damai", "url": href, "title": title, "artists": [title], "venue": "", "city": "Shanghai", "date_local": date_local, "timezone": "Asia/Shanghai", "status": status }) except Exception as e: print("[damai] error:", e) return out
